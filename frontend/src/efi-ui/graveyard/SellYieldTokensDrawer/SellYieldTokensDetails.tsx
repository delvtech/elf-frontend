 ...
 */
interface SellYieldTokenDetailsProps {
...
export function SellYieldTokenDetails({
  baseAssetSymbol,
  priceSlippageAndTradingFee,
  spotPriceBaseAssetForOneToken,
  unlockTimeStamp,
}: SellYieldTokenDetailsProps): ReactElement {
  const roundedYieldTokenPrice = spotPriceBaseAssetForOneToken?.toFixed(4);
    ...
    <div className={tw("flex", "flex-col", "space-y-6", "items-center")}>
      <LabeledText
        ...
        label={
          ...
          >{t`1 Yield Token ≈ ${roundedYieldTokenPrice} ${baseAssetSymbol}`}</span>
        }
      />
      <LabeledText
        muted={false}
